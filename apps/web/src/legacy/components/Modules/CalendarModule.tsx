...
const CalendarModule: React.FC = () => {
  ...
  return (
    <div className="h-full bg-white md:rounded-[2.5rem] md:shadow-2xl md:border border-slate-100 overflow-hidden flex flex-col text-right animate-fade-in" dir="rtl">
      <header className="p-5 md:p-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 bg-white sticky top-0 z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
           {isManager && (
             <div className="bg-slate-100 p-1 rounded-2xl flex shadow-inner w-full sm:w-auto">
                <button onClick={() => setViewMode('calendar')} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === 'calendar' ? 'bg-white shadow-md text-slate-900 border border-slate-200' : 'text-slate-400'}`}>
                   <List size={16} /> יומן
                </button>
                <button onClick={() => setViewMode('weekly-grid')} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === 'weekly-grid' ? 'bg-white shadow-md text-slate-900 border border-slate-200' : 'text-slate-400'}`}>
                   <Grid size={16} /> זמינות
                </button>
             </div>
           )}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-7 bg-slate-50 border-b border-slate-100">
          {dayNames.map(d => (
            <div key={d} className="p-4 text-center text-[11px] font-black uppercase tracking-widest text-slate-400">{d}</div>
          ))}
        </div>

        <div className="grid grid-cols-7 bg-slate-200 gap-[1px]">
          {blanks.map(b => <div key={`blank-${b}`} className="bg-slate-50/50 min-h-[120px] md:min-h-[160px]"></div>)}
          {days.map(d => {
             ...
             return (
               <div 
                 ...
                 className={`bg-white min-h-[120px] md:min-h-[160px] p-2 md:p-3 cursor-pointer hover:bg-slate-50 transition-all flex flex-col relative group ${isToday ? 'ring-2 ring-brand ring-inset' : ''}`}>
                  <div className="flex justify-between items-start flex-row-reverse mb-2">
                     <span className={`text-sm md:text-lg font-black w-7 h-7 md:w-9 md:h-9 flex items-center justify-center rounded-xl ${isToday ? 'bg-brand text-white shadow-lg' : 'text-slate-800'}`}>{d}</span>
                  </div>
                  
                  {viewMode === 'weekly-grid' && canSeeAvailability ? (
                    <div className="mt-1 flex flex-col gap-1 overflow-y-auto max-h-[100px] custom-scrollbar pr-1">
                        {availableStaff.map((u) => (
                            <div key={u?.id} className="text-[10px] font-bold bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg border border-indigo-100 truncate text-right shadow-sm" title={u?.name}>
                                {u?.name}
                            </div>
                        ))}
                    ...
                    <div className="mt-auto flex flex-col gap-1.5">
                        {countLessons > 0 && (
                          <div className="flex items-center gap-2 bg-blue-50 text-blue-600 px-2 py-1.5 rounded-xl border border-blue-100 shadow-sm w-full">
                            <BookOpen size={14} className="shrink-0" strokeWidth={2.5} />
                            <span className="text-[10px] font-black">{countLessons} שיעורים</span>
                          </div>
                        )}
                        {countActive > 0 && (
                          <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-2 py-1.5 rounded-xl border border-emerald-100 shadow-sm w-full">
                            <CheckCircle2 size={14} className="shrink-0" strokeWidth={2.5} />
                            <span className="text-[10px] font-black">{countActive} במשמרת</span>
                          </div>
                        )}
                    </div>
                  )}
      ...
      {selectedDay && selectedDayInfo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-fade-in" onClick={() => setSelectedDay(null)}>
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-8 md:p-12 overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
             <div className="flex justify-between items-start mb-8 flex-row-reverse">
                <div className="text-right">
                   <h3 className="text-3xl font-black text-slate-900 leading-tight">פירוט יום {new Date(selectedDay).toLocaleDateString('he-IL', { weekday: 'long', day: 'numeric', month: 'long' })}</h3>
                </div>
                <button onClick={() => setSelectedDay(null)} className="p-3 bg-slate-50 text-slate-400 rounded-2xl hover:text-slate-900 transition-all"><X size={28} /></button>
             </div>
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="flex flex-wrap gap-2 justify-end">
                        {selectedDayInfo.available.map(a => (
                           <div key={a.userId} className="bg-emerald-50 border border-emerald-100 px-4 py-2 rounded-xl text-emerald-900 font-bold text-sm flex flex-col items-center">
                ...
                <div className="space-y-4 pt-6 border-t border-slate-100">
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {selectedDayInfo.confirmed.map(s => (
                        <div key={s.id} className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl flex items-center justify-between flex-row-reverse">
                           <span className="font-black text-indigo-900">{s.userName}</span>
                ...
                <div className="space-y-4 pt-6 border-t border-slate-100">
                   ...
                   <div className="space-y-6">
                      {selectedDayInfo.lessons.map(l => (
                        <div key={l.id} className="bg-slate-50 border border-slate-100 p-6 rounded-[2rem] flex flex-col gap-4 text-right shadow-sm hover:shadow-md transition-shadow">
                           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-row-reverse">
                              <div className="flex items-center gap-4 flex-row-reverse">
                                 <div className="w-24 text-center font-black text-slate-900 tabular-nums text-xl flex flex-col bg-white p-2 rounded-2xl shadow-inner border border-slate-100">
                                    <span>{l.time}</span>
                                    {l.endTime && <span className="text-[10px] text-brand-ocean opacity-60">עד {l.endTime}</span>}
                                 </div>
                                 <div className="text-right">
                                    <div className="font-black text-slate-800 text-xl leading-tight">{l.clientName}</div>
                           ...
                           <div className="flex justify-between items-center border-t border-slate-200/50 pt-4">
                              <div className="text-[12px] font-black text-slate-500 tabular-nums bg-white px-3 py-1 rounded-lg">
                              ...
                              {l.instructorId && (
                                <div className="text-[11px] font-black bg-slate-900 text-white px-4 py-2 rounded-xl shadow-lg flex items-center gap-2">
