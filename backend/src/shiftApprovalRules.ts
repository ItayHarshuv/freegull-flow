/**
 * Approval window: after Saturday 18:00 local time, changes for the following
 * calendar week (Sun–Sat) require manager approval.
 */
...
/** Most recent Saturday 18:00 that is <= now. */
...
/** Sunday 00:00 of the week immediately after the given Saturday cutoff. */
export function getFollowingWeekRange(cutoffSaturday: Date): {
  start: Date;
  end: Date;
} {
...
export function requiresShiftChangeApproval(
  shiftDateStr: string,
  now = new Date()
): boolean {
