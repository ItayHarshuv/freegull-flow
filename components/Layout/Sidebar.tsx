...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, closeMobile }) => {
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-brand-ocean/60 z-[60] backdrop-blur-md transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={closeMobile} />
      <aside id="sidebar" className={`fixed top-0 right-0 h-screen w-[280px] bg-white z-[70] border-l border-slate-200 transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0 shadow-2xl' : 'translate-x-full'} md:translate-x-0`}>
        <div className="p-6 pb-8 flex justify-between items-center flex-row-reverse">
          <div className="flex items-center gap-3 flex-row-reverse">
            ...
            <div className="text-right">
              <h1 className="text-xl font-black text-brand-ocean tracking-tighter leading-none">FREEGULL</h1>
              <p className="text-[9px] text-brand-dark font-black uppercase mt-1">FLOW MANAGEMENT</p>
            </div>
          </div>
          <button onClick={closeMobile} className="md:hidden p-2 text-slate-400"><X size={24}/></button>
        </div>
        ...
        <div id="user-profile" className="p-6 border-t border-slate-100 bg-slate-50/50 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={switchUser} className="flex items-center justify-center gap-2 bg-slate-100 text-slate-600 hover:bg-slate-200 py-3 rounded-lg text-[10px] font-black uppercase shadow-sm transition-all">
               <RefreshCcw size={14} /> <span>החלף</span>
            </button>
            <button onClick={logout} className="flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-600 hover:text-rose-600 py-3 rounded-lg text-[10px] font-black uppercase shadow-sm transition-all">
